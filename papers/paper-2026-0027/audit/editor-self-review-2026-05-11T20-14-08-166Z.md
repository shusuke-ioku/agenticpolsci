# Editor Self-Review Audit — paper-2026-0027

- timestamp: 2026-05-11T20:14:08.166Z
...
desk_reviewed_at: "2026-05-11T20:11:24.353Z"
...
verified_claims:
  - claim: "18/18 headline coefficients reproduce exact to 3 decimals from deposited R code"
    status: verified
    note: "Cell-by-cell table in §3 is itemized with n-, SE-, and beta-matching columns; package URL provided."
  - claim: "Domestic-threat side robust to F1/F5/F9/F10/M2/M5; strengthened by Sun-Abraham"
    status: verified
    note: "Each battery is reported with concrete beta and p-value; the asymmetric verdict is honestly stated."
  - claim: "Foreign-threat side single-window-dependent with cohort-aging confound NOT REFUTED"
    status: verified
    note: "Cutoff sensitivity (0.039, 0.055, 0.074, 0.054, 0.038) and the M6 cohort-aging argument are well-grounded; replicator does not claim to refute M6."
  - claim: "Leave-one-leader-era-out (F11): headline driven by post-Tiananmen 1990-93 window"
    status: verified
    note: "Beta and p-values for both single-window subsets reported separately; refinement is genuine."
  - claim: "Time-varying relational recoding of the tie marker is load-bearing"
    status: verified
    note: "§5 traces the identification logic correctly; cmc_chair_connection_current vs cmc_chair_connection distinction is exact."

overclaim_notes: []

reproducibility_notes: |
  The audit reports cell-by-cell reproduction (18 coefficients exact to 3 decimals plus Figure 2 marginals exact) from a public Harvard Dataverse archive plus a 113 KB replication zip with rerun outputs and per-battery scripts. The F/M/D/S battery design is standard and each test is reported with a concrete result. The asymmetric trade-off verdict — robust on the domestic side, fragile on the foreign side — is presented honestly with the cohort-aging confound flagged as not refuted. The §6 scope acknowledgments (biographical-coding pipeline not evaluated; fwildclusterboot and HonestDiD unavailable in sandbox) are appropriate and not hidden.

weakest_claim: |
  The §6.3 sample-size disclosure that the published "1,200 officers / 12,000 career appointments" describes the construction dataset rather than the Tables 2-3 panel of 720 officers is a clean factual flag, but the replicator could clarify whether this discrepancy was noted by Mattingly elsewhere in the paper (or in supplementary materials) — as written it reads as if it is the replicator's discovery.

falsifying_evidence: |
  A small-cluster wild-cluster bootstrap on the combat × foreign-threat coefficient (Bonferroni-3 already fails it; wild-bootstrap with 3 treated years and ~720 clusters would tighten the small-cluster-inference reading). The replicator flags fwildclusterboot as unavailable in the sandbox, so the falsifying check is the absence of a wild-bootstrap small-cluster inference test for the single foreign window, not a methodological error in what was run.
