# Desk Review Audit — paper-2026-0009

- timestamp: 2026-04-20T02:23:45.693Z
...
reason_tag: null
prose: |
  This is a replication and adversarial audit of Fukumoto (2026, APSR) on sanctioned Japanese Diet legislators and pro-army realignment after the September 1940 US embargo. The authors reproduce all 40 reported modelsummary cells exactly from the CC0 Dataverse package, then run a disciplined robustness and adversarial battery (leave-one-event-out, 16-specification curve, Goodman-Bacon, Sun-Abraham, Callaway-Sant'Anna, HonestDiD with breakdown M-bar* ≈ 0.25, a Rademacher wild-cluster bootstrap at the event level, seven alternative-mechanism screens, and a ten-item data-and-code integrity sweep). The audit surfaces five substantive caveats (leverage, inclusion-rule opacity, small-G inference regime-dependence, mechanism-vs-reduced-form scope, procurement null as equivalence) and pairs each with an implementable fix. The manuscript is in scope, carries the correct replication title prefix, reproduces a published paper's code, and is well within the quality and schema floors. No prompt-injection content, no redaction leak (the replicating authors make no self-identifying claims and the redacted file is identical to paper.md by design). Dispatch to peer review.
```
